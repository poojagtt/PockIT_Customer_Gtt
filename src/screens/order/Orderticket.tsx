...
import {Icon, TextInput, Header} from '../../components';
...
interface OrderticketProps extends OrderRoutes<'Orderticket'> {}
...
const Orderticket: React.FC<OrderticketProps> = ({navigation, route}) => {
  const colors = useTheme();
  ...
  const {t} = useTranslation();
  const {jobItem, orderId, type} = route.params;
  ...
  const isWarrantyValidForService = () => {
  const jobCompletedDate = moment(jobItem.JOB_COMPLETED_DATETIME, 'YYYY-MM-DD HH:mm:ss');
  const warrantyExpiryDate = jobCompletedDate.clone().add(jobItem.WARRANTY_PERIOD, 'days');
  return moment().isBefore(warrantyExpiryDate);
};
  const isWarrantyValidForProduct = () => {
  const jobCompletedDate = moment(jobItem.orderData.DELIVERY_DATE, 'YYYY-MM-DD');
  const warrantyExpiryDate = jobCompletedDate.clone().add(jobItem.detailsData[0].WARRANTY_PERIOD, 'days');
  return moment().isBefore(warrantyExpiryDate);
};
  const manageTickets = async () => {
    ...
    try {
      ...
      if (type === 'S') {
        filterQuery = ` AND SHOP_ORDER_ID = ${orderId}`;
      } else {
        filterQuery = ` AND ORDER_ID = ${jobItem.ORDER_ID} AND JOB_CARD_ID = ${jobItem.JOB_CARD_ID}`;
      }
      if (selectedFilter) {
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: colors.background}} edges={['top', 'bottom']}>
      <View style={{flex: 1}}>
        ...
        <View
          ...
          <Header
            label={t('order.manageTickets.title')}
       ...
       { ((type=="J" && isWarrantyValidForService()) || (type=="S" && isWarrantyValidForProduct()) )   &&<TouchableOpacity
