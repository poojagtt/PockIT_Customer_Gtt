...
interface CreateTicketsProps extends MenuRoutes<'CreateTickets'> {}
    ...
    return (
      <>
        ...
        <Modal visible={true} transparent={false} onRequestClose={onClose}>
          ...
          >
              <Header label={'Create Support Ticket'} onBack={() => onClose()} />
              <View style={{flex: 1, backgroundColor: colors.background}}>
                ...
                {ticket.show ? (
                         ...
                         <ImagePicker
                          ...
                          {ticket.attachment ? (
                            <View style={{position: 'relative'}}>
                              ...
                              <TouchableOpacity
                                onPress={() =>
                                  setTicket({...ticket, attachment: null})
                                }
                                style={{
                                  position: 'absolute',
                                  top: 10,
                  ...
                  <View
                    style={[
                      styles.footerContainer,
                      {backgroundColor: colors.white,paddingHorizontal:Platform.OS=='ios'?20:20},
                    ]}>
                    <View style={styles.footerContent}>
                      <Icon
...
const CreateTickets: React.FC<CreateTicketsProps> = ({navigation, route}) => {
  const colors = useTheme();
  const {ticketGroup} = route.params;
  const [loading, setLoading] = useState<boolean>(false);
      ...
      apiCall
        ...
        .then(res => {
          if (res.data.data.length > 0) {
            ...
            apiCall
              .post(`api/ticketGroup/get`, {
                filter: ` AND PARENT_ID=${head.ID} AND TYPE='O' AND TICKET_TYPE='C' AND STATUS = 1 `,
                sortKey: 'SEQ_NO',
                sortValue: 'ASC',
              })
              .then(res => {
  ...
  return (
    <SafeAreaProvider style={{flex: 1, backgroundColor: colors.background}}>
      <View style={{flex: 1}}>
        <Header label={'Support Tickets'} onBack={() => navigation.goBack()} />
