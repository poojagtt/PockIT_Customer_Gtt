...
import {IMAGE_URL, useTheme, fontFamily} from '../../modules';
...
interface SubCategoryProps extends HomeRoutes<'SubCategory'> {}
const SubCategory: React.FC<SubCategoryProps> = ({navigation, route}) => {
  ...
  useEffect(() => {
    setSubCategory(
      item.subCategories.map(value => {
        return {...value, CATEGORY_ID: item.ID, CATEGORY_NAME: item.NAME};
      }),
    );
