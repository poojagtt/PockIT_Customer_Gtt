...
interface ServiceListProps extends HomeRoutes<'ServiceList'> {}
const ServiceList: React.FC<ServiceListProps> = ({navigation, route}) => {
  ...
  async function getService(): Promise<void> {
    ...
    const res = await apiCall
      .post('order/getServices', payload)
      .then(res => res.data);
        ...
        {loading.loader ? (
          ...
          <View style={{flex: 1}}>
            <FlatList
              ...
              renderItem={({item}) => {
                console.log("item.IS_PARENT",item)
                if (item.IS_PARENT) {
                  return (
                    <CatLogTiles
                      item={item}
