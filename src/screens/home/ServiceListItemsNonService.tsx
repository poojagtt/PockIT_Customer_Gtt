...
const ServiceListItemsNonService: React.FC<ServiceListProps> = ({
  navigation,
  route,
}) => {
  ...
  const [selectedItem, setSelectedItem] = useState<ServicesInterface | null>(
    null,
  );
  ...
  console.log('serviceInfoModal', serviceInfoModal.service);
  async function getService(): Promise<void> {
    ...
    const res = await apiCall
      .post('services/get', {
        filter: `AND STATUS=1 AND SUB_CATEGORY_ID=${subCategory.ID} AND PARENT_ID=${service?.SERVICE_ID} AND (SERVICE_TYPE= 'O' OR SERVICE_TYPE= 'C' OR SERVICE_TYPE IS NULL)`,
        sortKey: 'ID',
        sortValue: 'asc',
      })
      .then(res => res.data);
      ...
      <View style={{flex: 1}}>
        ...
        {serviceInfoModal.service && serviceInfoModal.show ? (
          <ServiceInfoModal
            ...
            onConfirm={quantity => {
              console.log('quntity from child', quantity);
...
export default ServiceListItemsNonService;
