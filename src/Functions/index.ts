...
const OrderFileName = (orderNumber: string) => {
  return orderNumber.replace(/\//g, '_') + '.pdf';
};

const formatPeriod = (days: any) => {
  if (days <= 0) return '';
  ...
  if (months === 0) {
    ...
    return `${monthText} ${remainingDays} ${
      remainingDays === 1 ? 'day' : 'days'
    }`;
  }
};

export {getCountryCode, OrderFileName, formatPeriod};
